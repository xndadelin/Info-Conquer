...
const compilerCPP = (code, idSolution, language) => {
    ...
    fs.writeFileSync(`${idSolution}/test.cpp`, code);
    const fileMemory = fs.statSync(`${idSolution}/test.cpp`).size;
    try {
        let compilationResult = null;
        try{
            //built the container => compile the code
            execSync('docker build --no-cache -t cpp-image .', {cwd: idSolution})
            //check for compilation errorsb
            compilationResult = execSync(`docker run -i cpp-image cat compilation_error.txt > ${idSolution}/compilation_error.txt`)
